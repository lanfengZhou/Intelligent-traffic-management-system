	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			if (rowid === "new") {
				...
				frmoper = "add";
				p.caption="新用户注册";
				jQuery("#list1").jqGrid('hideCol', ["sum"]);//隐藏操纵
			} 
			else if(infors==="revise"){
				p.caption="修改用户信息";
				jQuery("#list1").jqGrid('hideCol', ["sum"]);//隐藏操纵
			}
			else if(infors==="recharge"){
				p.caption="用户充值";
				//p.addClass(".money");
			}
			else {
			...
			var closeovrl = true;//设置弹出框的提交取消翻页按钮
